...
class userRegisterForm(forms.Form):
    ...
    def clean(self):
        ...
        if first_name and last_name and username and password and domain and email and confirm_password:
            
            user_exists = users.objects.filter(username=username, domain = domain).exists()
            if user_exists:
                errors['username'] = "Username already taken"
                
            domain_exists = registeredDomains.objects.filter(domain = domain, approved = True).exists()
